...
! Puzzle for Day 05 part 02 : 
! Build with:  gfortran -static-libgcc -o aoc_day05_p2 aoc_day05_p2.f90

program aoc_day05_p2

	use, intrinsic :: iso_fortran_env
	implicit none

	! Declare and set constants the AOC day and part
	integer,parameter :: day = 5, part = 2
	...
	integer, allocatable :: p2_intersect(:,:)
	...
	do i=1,total_lines
		if (begin_xy(1,i) .eq. end_xy(1,i)) then
			...
			! PART 2
			do m = 0, abs(begin_xy(1,i) - end_xy(1,i))
				p2_intersect(begin_xy(1,i) + (m * j), begin_xy(2,i) + (m * k)) = &
				p2_intersect(begin_xy(1,i) + (m * j), begin_xy(2,i) + (m * k)) + 1
			end do
		end if
	end do
	
	write (*,'(A,I0)') " » Horizontal and vertical lines where > two vent lines cross (Part 1): ", count(p1_intersect .gt. 1)
	write (*,'(A,I0)') " » Points where all vent lines cross (Part 2): ", count((p1_intersect + p2_intersect) .gt. 1)
